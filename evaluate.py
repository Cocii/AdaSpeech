...
def evaluate(d_model, model, step, configs, logger=None, vocoder=None):
    ...
    for batchs in loader:
        for batch in batchs:
            ...
            with torch.no_grad():
                ...
                loss_sums.append(adv_loss.item() * len(batch[0]))
                loss_sums.append((d_loss_r + d_loss_f).item() * len(batch[0]))

    loss_means = [loss_sum / len(dataset) for loss_sum in loss_sums]
    message = "Validation Step {}, Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Pitch Loss: {:.4f}, Energy Loss: {:.4f}, Duration Loss: {:.4f}, Mel_Level Loss: {:.4f}, Discriminator Loss: {:.4f}, Adversarial Loss: {:.4f}".format(
